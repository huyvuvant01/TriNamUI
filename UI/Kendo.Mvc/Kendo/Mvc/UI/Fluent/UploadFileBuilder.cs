// Decompiled with JetBrains decompiler
// Type: Kendo.Mvc.UI.Fluent.UploadFileBuilder
// Assembly: Kendo.Mvc, Version=2021.3.1109.0, Culture=neutral, PublicKeyToken=121fae78165ba3d4
// MVID: 21B8CA96-D514-4435-8358-0B5915A38622
// Assembly location: D:\TriNam\Git\TriNamUI\TriNamUI\TestTelerikNetCore\bin\Debug\net6.0\Kendo.Mvc.dll

namespace Kendo.Mvc.UI.Fluent
{
  public class UploadFileBuilder
  {
    public UploadFileBuilder(UploadFile container) => this.Container = container;

    protected UploadFile Container { get; private set; }

    public UploadFileBuilder Extension(string value)
    {
      this.Container.Extension = value;
      return this;
    }

    public UploadFileBuilder Name(string value)
    {
      this.Container.Name = value;
      return this;
    }

    public UploadFileBuilder Size(double value)
    {
      this.Container.Size = new double?(value);
      return this;
    }
  }
}
